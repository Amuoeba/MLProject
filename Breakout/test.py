...
def test(rank, params, shared_model):
    ...
    while True: # repeat
        ...
        if done: # synchronizing with the shared model (same as train.py)
            ...
            with open("./DUMPS/model_DUMP2.txt","wb") as file:
            ...
            cx2 = Variable(cx2.data, volatile=True)
            hx2 = Variable(hx2.data, volatile=True)
            
        value, action_value, (hx, cx),(hx2,cx2) = model((Variable(state.unsqueeze(0), volatile=True), (hx, cx),(hx2,cx2)))
        prob = F.softmax(action_value,1)
        ...
        if done: # printing the results at the end of each part
            ...
            REWARDS.append({"time":time.strftime("%Hh %Mm %Ss",time.gmtime(time.time() -start_time)),"reward":reward_sum,"ep-len":episode_length})
            with open("./DUMPS/reward_DUMP2.txt","wb") as f:
